...
class AccountRejectAdapter(private val rejectedList: List<RejectedData>) :
    RecyclerView.Adapter<AccountRejectAdapter.RejectedViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RejectedViewHolder, position: Int) {
        ...
        holder.label.text = "Account Verification Rejected"
        holder.recTitle.text = "Reason: ${currentRejected.rejectReason}"
        holder.dateandTime.text ="Since: ${currentRejected.rejectTimestamp}"
    ...
    data class RejectedData(
        val rejectReason: String,
        val rejectTimestamp: String
    )
}
